...
export default function TelegramStatus({
    ...
    const sendPassword = () => {
        ...
        mAxios.post("/send/telegram/password", {
            password
        }).then(res => {
            setTwoFactor(false);
            setLoading(true);
    ...
    const sendCode = () => {
        ...
        mAxios.post("/send/telegram/code", {
            code
        }).then(res => {
            setLoading(true);
            setCodeShow(false);
    ...
    useEffect(() => {
        console.log("this is p afasfd")
        const token = localStorage.getItem("accessToken");

        const source = new EventSource(process.env.NEXT_PUBLIC_API_BASE_URL + '/api/get/telegram/status?token=' + token);
        ...
        <>
            {
                !loading ?
                    <Box>
                        {
                            codeShow ?
                                <Box>
                                    ...
                                    <Typography>
                                        لطفا کد تاییدی که به حساب تلگرام شما ارسال شده را وارد کنید
                                    </Typography>
                                    <Box sx={{ my: "10px" }}>
                                    </Box>
                                    <CustomTextField
                    ...
                    <>
                        <Typography sx={{ mb: 4 }}>
                            در حال بررسی وضعیت حساب تلگرام شما
                        </Typography>
